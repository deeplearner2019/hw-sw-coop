...
def parse_resources_from_config(config):
    ...
    for resource_name, resource_opts in config["resources"].iteritems():
        ...
        if 'main_file' in resource_opts:
            main_file = resource_opts['main_file']
        
        if 'main_file' in resource_opts:
            resources[resource_name] = Resource(resource_name, tasks, scheduler_object, scheduler_class, max_concurrent, max_finished_jobs,main_file)
        else:
        ...
        logging.debug('  resource main file : %s' % ', '.join(resources[resource_name].main_file))
        logging.debug('')
