...
from nn_search.nets import cifarnet
...
tf.app.flags.DEFINE_string(
    'dataset_dir', '/home/lile/dataset/cifar10_val', 'The directory where the dataset files are stored.')
...
tf.app.flags.DEFINE_integer('max_number_of_steps', 14062,
                            'The maximum number of training steps.') # follow the paper, we train for 20 epochs, so the number of steps can be computed as 20 * train_image_num (45000) / batch_size (32) = 28125
...
def lock_process(cmd,filename,lock_name):
    ...
    cmd_s = cmd    
    command_file = os.path.join(PATH_TO_HACONE_LOCAL,'Spearmint-PESM','examples','cifar10','command_files',filename+'.sh')
    
    save_dir = os.path.join(PATH_TO_HACONE_LOCAL,'Spearmint-PESM','examples','cifar10','command_files')
...
def test_movidius(model_name, case):
    subprocess.call("python {0}/tensorflow/nn_search/export_inference_graph_movidius.py \
     --job_name=cifar10_movidius \
     --name_job={1} \
     --output_file={0}/outputs/{2}/{1}/inference_graph.pb \
     --PATH_TO_HACONE={0}".format(PATH_TO_HACONE_LOCAL, model_name, case),shell=True)
 
     #Freeze the weights in the graph in a protobuf file
    subprocess.call("python {0}/tensorflow/nn_search/freeze_graph_14.py \
     --input_graph={0}/outputs/{3}/{1}/inference_graph.pb \
     --input_checkpoint={0}/outputs/{3}/{1}/model.ckpt-{2} \
     --input_binary=true \
     --output_graph={0}/outputs/{3}/{1}/frozen_graph.pb \
     --output_node_names=CifarNet/Predictions/Reshape_1".format(PATH_TO_HACONE_LOCAL, model_name, 14062, case),shell=True)

    dico_hardware = {}

    path_to_movidius = '/home/lile/Projects/git_repo/hacone/movidius'
    model_dir = '{}/outputs/{}/{}'.format(PATH_TO_HACONE_LOCAL, case, model_name)
    log_file = '{}/measurements/{}/{}.profile'.format(PATH_TO_HACONE_LOCAL, case, model_name)
    ...
    #Retrieve power
    result_file = '{}/measurements/{}/{}.csv'.format(PATH_TO_HACONE_LOCAL, case, model_name)
    ...
    #Save results
    with open(PATH_TO_HACONE_LOCAL + '/measurements/{}/{}.txt'.format(case, model_name), 'w') as my_file:
        json.dump(dico_hardware, my_file)

    return dico_hardware

def create_name_dic(params):
    '''
    ...
    for i in xrange(0, 5):
        name += '_'
        name += str(params['b{}_i1'.format(i)][0])
        name += '_'
        name += str(params['b{}_i2'.format(i)][0])
        name += '_'
        name += str(params['b{}_o1'.format(i)][0])
        name += '_'
        name += str(params['b{}_o2'.format(i)][0])
    print(name)
    return name
...
def main(job_id, params):
    ...
    model_name = create_name_dic(params) 
    case = 'cifarnet_movidius'
    output_dir = os.path.join(PATH_TO_HACONE_LOCAL, 'outputs', case, model_name) #_parse_flags() is called when first access FLAGS's attibute

    measure_dir = os.path.join(PATH_TO_HACONE_LOCAL, 'measurements', case)
    ...
    for i in xrange(0, 5):
        candidate.append(params['b{}_i1'.format(i)][0])
        candidate.append(params['b{}_i2'.format(i)][0])
        candidate.append(params['b{}_o1'.format(i)][0])
        candidate.append(params['b{}_o2'.format(i)][0])
    print(candidate)
    
    N = 2
    F = 24
    train_model(candidate, N, F)
    f = eval_model(candidate, N, F)
    
    file_to_send = "{}/jobs/job{}.txt".format(PATH_TO_HACONE_LOCAL, model_name)
    dico_to_save = {}
    ...
    return {"f": float( 1.0 - f), "time":dico_hardware['time'], "energy":dico_hardware['energy']}
