...
instance_id = 'i-0767369e1f3622746' #p2.16xlarge
public_DNS = 'ec2-174-129-111-182.compute-1.amazonaws.com'
...
def is_already_trained(name):
    done_jobs = subprocess.check_output('ssh arturo@10.217.128.217 ls /home/data/arturo/models_trained',shell=True)
...
def lock_instance_gpu(command,filename,file_to_get_back):
    ...
    cmd_s = '#!/bin/bash\n(flock -w 3600 9 || exit 1; cd {0}/movidius; export PYTHONPATH={0}/movidius:$PYTHONPATH;python {1}; sleep 5) 9>/var/lock/p28_3'.format(PATH_TO_HACONE,python_file)
...
def main(job_id, params):
    ...
    if not is_already_trained(name):
        ...
        subprocess.call("scp -r {0}/Spearmint-PESM/examples/cifar10/output_0/{1} arturo@10.217.128.217:/home/data/arturo/models_trained/{1}".format(PATH_TO_HACONE,name), shell = True)
