...
instance_id = 'i-00ca14891b4644e36' #p2.8xlarge
public_DNS = 'ec2-54-84-73-1.compute-1.amazonaws.com'
...
def listen_to_remote_process(file_path):
    ...
    while True:
        ...
        if exists_remote(file_path) or timer > 20: # more than 3 hours
            break
...
def main(job_id, params):
    ...
    subprocess.call("scp {}@{}:/home/data/arturo/hardware_metrics/hardware_metrics_{}.txt {}/Spearmint-PESM/examples/cifar10/output_0/{}/hardware_metrics_titanX.txt ".format(user, server_DNS, job_id, PATH_TO_HACONE,name), shell=True)
    with open('{}/Spearmint-PESM/examples/cifar10/output_0/{}/hardware_metrics_titanX.txt'.format(PATH_TO_HACONE,name)) as my_file:
        dico_hardware = json.load(my_file)
    for key, value in dico.items():
        dico_hardware["{}".format(key)] = value


    with open('{}/Spearmint-PESM/examples/cifar10/output_0/{}/hardware_metrics_titanX.txt'.format(PATH_TO_HACONE,name), 'w') as my_file:
