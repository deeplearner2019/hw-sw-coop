...
def cifarnet(images, candidate, N, F, num_classes=10, is_training=False,
  ...
  with tf.variable_scope(scope, 'CifarNet', [images]):
   ...
   # net = slim.avg_pool2d(net, [8,8], stride=1, scope = 'global_pool')
    end_points['global_pool'] = net
   
    shape = net.get_shape()
    print("final output shape")
    print(shape)
    net = tf.reshape(net,[-1,shape[-1]])
...
operations = {}
operations['0'] = '3x3_max_pool'
operations['1'] = 'identity'
operations['2'] = '3x3_depth_conv'
operations['3'] = '3x3_conv'
operations['4'] = '5x5_depth_conv'
operations['5'] = '5x5_conv'
operations['6'] = '7x7_depth_conv'
operations['7'] = '7x7_conv'
...
def apply_operation(net, op_name, num_outputs, stride = 1, scope = None, is_training=False):
   with tf.variable_scope(scope):
    ...
    if op_name == '3x3_conv':
       ...
       return net
    if op_name == '5x5_conv':
       net = slim.conv2d(net, num_outputs, [5, 5], stride = stride, activation_fn = tf.nn.relu, normalizer_fn = slim.batch_norm, normalizer_params={'is_training':is_training}, scope = op_name )
       return net
    if op_name == '7x7_conv':
       net = slim.conv2d(net, num_outputs, [7, 7], stride = stride, activation_fn = tf.nn.relu, normalizer_fn = slim.batch_norm, normalizer_params={'is_training':is_training}, scope = op_name)
       return net

    if op_name == 'identity':
       if net.get_shape().as_list()[3] == num_outputs:
           print('\nIdentity subsample used\n')
           net = subsample(net, stride, scope = op_name+'_subsample')
       else:
           print('\nIdentity conv2D\n')
...
def build_cell(end_points, cell_id, candidate, num_outputs, stride = 1, scope = None, is_training=False):
    ...
    with tf.variable_scope(scope):
       for i in xrange(B):
          ...
          # the output dimension can be different 
          ...
          #input_points[str(i)] = tf.identity(input_points[str(i)])
          #input_points[str(i)] = slim.max_pool2d(input_points[str(i)],[1,1],stride=1,scope='fake_pool')
