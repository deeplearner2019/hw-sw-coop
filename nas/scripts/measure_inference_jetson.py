...
import measure # module to monitor the Jetson TX2
import pickle
INTERVAL = 0.02
pl = measure.PowerLogger(interval=INTERVAL, nodes=measure.getNodesByName(['module/gpu']))

tf.reset_default_graph()
slim = tf.contrib.slim

PATH_TO_HACONE = '/home/nvidia/Projects/hacone'
...
tf.app.flags.DEFINE_string(
    'dataset_dir', '/home/nvidia/dataset/cifar10_val', 'The directory where the dataset files are stored.')
...
def eval_model(candidate, N, F, save_dir, model_name):
  ...
  with tf.Graph().as_default():
    ...
    pickle.dump(data_list, open(os.path.join(save_dir, model_name + '_data_list.pkl'),'wb'))
...
def get_start_end(power_list, thresh = 1000):
    ids = [i for i, p in enumerate(power_list) if p > thresh]
    return min(ids), max(ids)

def integrate_power(power_list, time_list, start, end):
    energy = 0
    for t in range(start + 1, end):
       energy += power_list[t] /1000. * (time_list[t] - time_list[t-1])
    return energy

def main(model_name, case, measure_dir):
    with open(PATH_TO_HACONE + '/jobs/job{}.txt'.format(model_name)) as fp:
        data = json.load(fp)

    job_id = data['job']
    params = data['params']
    #params = json.loads(params) # no idea why this is needed
    output_dir = os.path.join(PATH_TO_HACONE, 'outputs', model_name) #_parse_flags() is called when first access FLAGS's attibute
    ...
    duration, energy = eval_model(candidate, N, F, measure_dir, model_name)
    dico = {}
    dico["time"] = duration
    dico["energy"] = energy

    measure_dir = PATH_TO_HACONE + '/measurements/{}'.format(case)
    with open(os.path.join(measure_dir, model_name + '.txt'), 'w') as fp:
        json.dump(dico, fp)
...
if __name__ == "__main__":
    ...
    main(model_name, case, measure_dir)
