...
def inference_task(model_name, case, N, F):
    """
    ...
    subprocess.call('python {}/scripts/inference_titan_x_final.py {} {} {} {}'.format(PATH_TO_HACONE, model_name, case, N, F), shell=True)
...
def main(model_name, case, N, F):  
    inference_thread = threading.Thread(target = inference_task, kwargs ={'model_name':model_name,'case':case, 'N':N, 'F':F})
    ...
    with open(os.path.join(save_dir, model_name + '_final_{}_{}.txt'.format(N,F)),'w') as f:
        json.dump(dico, f)

    pickle.dump(power_list, open(os.path.join(save_dir, model_name + '_power_list_final_{}_{}.pkl'.format(N,F)),'wb'))
