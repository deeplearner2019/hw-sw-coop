...
if __name__ == '__main__':
    ...
    bitwidth = 3
    delayQuant = 1000
    SAVE_PATH = "./model-4bit-inputQuant/"
    train_f, test_f = ['./har-%s-quant4.tfrecords' % i for i in ['train', 'eval']]
    ...
    logits= create_model(data_x,2,isTraining=phase,NUM_CLASSES=NUM_CLASSES)
#         #FIXME
#         bn3 = tf.layers.batch_normalization(result, training=isTraining)
        ...
        if args.eval:
            ...
            for e in range(start_epoch, epochs):
                ...
                for i in range(0, nBatchs):
                     #, cur_conv1, cur_conv2, cur_conv3 #, conv1, conv2, conv3
                    _, i_global, cur_loss, cur_train_eval, summary, cur_input\
                    = sess.run([optimizer, global_step, loss, eval_op, summary_op_train, data_x],feed_dict={handle: handle_train, drop_prob: 0.5, learning_rate: lr(e+1), phase: True})
                    # print i_logits
