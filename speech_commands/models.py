...
def prepare_model_settings(label_count, sample_rate, clip_duration_ms,
                           ...
                           preprocess,first_height,first_width,second_height,second_width,third_height,third_width,no_of_layers):
  ...
  return {
      ...
      'first_weights_h': first_height,
      'second_weights_h':second_height,
      'first_weights_w':first_width,
      'second_weights_w':second_width,
      'third_weights_h':third_height,
      'third_weights_w':third_width,
      'layers':no_of_layers
  }
...
def create_conv4_model(fingerprint_input,model_settings,is_training,WEIGHT_DECAY=1e-3):
   ...
   with tf.variable_scope('conv1') as scope:
        conv1 = tf.layers.conv2d(
            ...
            filters=int(32),
            kernel_size=[model_settings['first_weights_h'],model_settings['first_weights_w']],
            padding='VALID',
            use_bias=False,
        ...
        pool1 = tf.layers.MaxPooling2D(2, 2, padding='same')(relu1)
   with tf.variable_scope('conv2') as scope:
        conv2 = tf.layers.conv2d(
            inputs=pool1,
            filters=int(64),
            kernel_size=[ model_settings['second_weights_h'], model_settings['second_weights_w'] ],
            padding='SAME',
            use_bias=False,
            kernel_regularizer=tf.contrib.layers.l2_regularizer(WEIGHT_DECAY)
        )
        ...
        pool2 = tf.layers.MaxPooling2D(2, 2, padding='same')(relu2)
        if is_training:
            drop2= tf.layers.dropout(pool2,rate=dropout_prob)
   ...
   with tf.variable_scope('conv3') as scope:
        conv3 = tf.layers.conv2d(
            inputs=drop2,
            filters=int(64),
            kernel_size=[model_settings['third_weights_h'],model_settings['third_weights_w']],
            padding='SAME',
            use_bias=True,
            kernel_regularizer=tf.contrib.layers.l2_regularizer(WEIGHT_DECAY)
        )
        ...
        if is_training:
            drop3= tf.layers.dropout(relu3,rate=dropout_prob)
   ...
   if model_settings['layers']==3:
        with tf.variable_scope('fully_connected') as scope:
            flat = tf.layers.flatten(drop3)
            
            logits = tf.layers.dense(inputs=flat, units=model_settings['label_count'], name=scope.name, use_bias=False, kernel_regularizer=tf.contrib.layers.l2_regularizer(WEIGHT_DECAY))
   if model_settings['layers']==2:
        with tf.variable_scope('fully_connected') as scope:
            flat = tf.layers.flatten(drop2)
            
            logits = tf.layers.dense(inputs=flat, units=model_settings['label_count'], name=scope.name, use_bias=False,                                     kernel_regularizer=tf.contrib.layers.l2_regularizer(WEIGHT_DECAY))
   if model_settings['layers']==4:
        with tf.variable_scope('fully_connected') as scope:
            flat = tf.layers.flatten(relu4)
            if is_training:
                flat= tf.layers.dropout(flat,rate=dropout_prob)
    
       
            logits = tf.layers.dense(inputs=flat, units=model_settings['label_count'], name=scope.name, use_bias=False, 
kernel_regularizer=tf.contrib.layers.l2_regularizer(WEIGHT_DECAY*4))
   if is_training:
...
def create_conv_model(fingerprint_input, model_settings, is_training):
  ...
  fingerprint_4d = tf.reshape(fingerprint_input,
                              [-1, input_time_size, input_frequency_size, 1])
  first_filter_width = model_settings['first_weights_h']
  first_filter_height = model_settings['first_weights_w']
  ...
  second_filter_width = model_settings['second_weights_h']
  second_filter_height = model_settings['second_weights_w']
  ...
  third_filter_width = model_settings['third_weights_h']
  third_filter_height = model_settings['third_weights_w']
  third_filter_count = 92
  second_weights = tf.get_variable(
      name='third_weights',
      initializer=tf.truncated_normal_initializer(stddev=0.01),
      shape=[
  ...
  third_bias = tf.get_variable(
      name='third_bias',
      initializer=tf.zeros_initializer,
      shape=[third_filter_count])
  third_conv = tf.nn.conv2d(second_dropout, second_weights, [1, 1, 1, 1],
                             'SAME') + third_bias
